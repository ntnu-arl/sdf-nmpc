...
class Quad(BaseModel):
    def __init__(self, cfg):
        ...
        theta_z = cs.atan2(q[3], q[0])
        qyaw = cs.vertcat(cs.cos(theta_z), 0, 0, cs.sin(theta_z))
        yaw = quat2yaw(q)
        v = self.x[7:]
        gamma = self.u[0] * self.cfg.robot.limits.gamma
        roll = self.u[1] * self.cfg.robot.limits.roll
        pitch = self.u[2] * self.cfg.robot.limits.pitch
        wz = self.u[3] * self.cfg.robot.limits.wz

        ## dynamics
        V_R_B = euler2rot(cs.vertcat(roll, pitch, 0))
        W_R_V = quat2rot(qyaw)
        W_R_B = W_R_V @ V_R_B
        ...
        self.u_to_TRPYr = cs.Function('u_to_TRPYr', [self.x, self.u, self.p], [gamma * self.cfg.robot.mass, roll, pitch, wz])

        ## cost
        q_d = self.p[self.cfg.mpc.p_idx.q_d]
        q_e = hamilton_prod(q_d, invert(q))
        self.y = cs.vertcat(p, q_e[3], v, roll, pitch, wz, W_a[2])
